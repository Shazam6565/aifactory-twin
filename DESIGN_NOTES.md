...
`README.md` says what this repo does. `ARCHITECTURE.md` gives the current, as-built V1 story —
read that first. This document says **why it is shaped this way**, at the level of detail that
lets every ordering decision be defended rather than merely stated.
...
**V1 note:** the published rack component currently authors geometry as ASCII `geo.usda`
rather than binary `geo.usdc`, since the placeholder geometry is small enough that the
diffability argument dominates. Revisit if real vendor meshes make ASCII geometry unwieldy.
